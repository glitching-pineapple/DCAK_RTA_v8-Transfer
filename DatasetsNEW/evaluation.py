...
def evaluate_sample(
    ...
    if DATASET == "gsm8k":
        ...
        question = sample['question']
        options_raw = sample.get('options', sample.get('choices',{}))
        if isinstance(options_raw,dict):
            choices = [options_raw[k] for k in sorted(options_raw.keys())]
    ...
    # Extract verbalized confidence from CoT response (1-10 scale)
    ...
    result = {
        ...
        "seq_confidence_mean": confidence_metrics["log_prob_sum"] / max(len(token_probs),1),
        ...
        # Verbalized confidence (1-10 scale)
