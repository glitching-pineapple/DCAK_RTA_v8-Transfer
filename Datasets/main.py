...
def main():
    ...
    # Load model - pin to GPU 0 (7B fp16 ≈ 14GB, fits on one 40GB A100)
    model, tokenizer = load_model_and_tokenizer(model_device="cuda:0")
    
    # Initialize semantic entropy calculator if enabled
    # Pin NLI model to GPU 1 so it doesn't compete with the main model
    ...
    if COMPUTE_SEMANTIC_ENTROPY:
        ...
        nli_device = "cuda:1" if torch.cuda.device_count() > 1 else "cuda:0"
    ...
    if result.get('verbalized_confidence') is not None:
        print(f"Verbalized confidence: {result['verbalized_confidence']:.2f}")
    else:
